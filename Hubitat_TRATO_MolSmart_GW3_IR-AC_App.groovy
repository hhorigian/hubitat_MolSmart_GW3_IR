/**
 *  MolSmart GW3-IR para AC (Ar Condicionado) App for Hubitat
 *
 *  Copyright 2024 VH
 *
...
definition(
    name: "MolSmart GW3-IR AC App ",
    namespace: "TRATO",
    author: "VH",
    description: "MolSmart GW3-IR para AC para Hubitat",
...
def initialize() {
    ...
    for(int i = 1; i<=inputCount; i++) {
        def contactName = "MolGW3-IR-" + Integer.toString(i) + "_${app.id}"
	    logDebug "initialize(): adding driver = " + contactName
        
        def contactDev = getChildDevice(contactName)
	    if(!contactDev) contactDev = addChildDevice("TRATO", "MolSmart GW3 - IR - AC", contactName, null, [name: "MolSmGW3 IR " + Integer.toString(i), inputNumber: thisName])
