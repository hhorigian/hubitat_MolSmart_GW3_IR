/**
 *  MolSmart GW3-IR Universal Remote Learning) via iDoor App for Hubitat
 *
 *  Copyright 2024 VH
 *
 ...
 *
 *   +++  Criar botões no Controle Universal no iDoor e enviar via post. 
 *
*
*/

definition(
    name: "MolSmart GW3-IR Universal(Learning) App",
    namespace: "TRATO",
    author: "VH",
    description: "MolSmart GW3-IR Universal via Idoor ",
    category: "Remote Controls / IR / TV / AC",
    iconUrl: "",
    iconX2Url: "")
...
def mainPage() {
    ...
    section("MolSmart GW3 (IR) Setup Configuration") {
    	...
    	input name: "numberOfButtons", title:"Qtde de Botões", type: "string", required: true, defaultValue: "10"       
...
def initialize() {
    ...
    for(int i = 1; i<=inputCount; i++) {
        def contactName = "MolGW3-IR-UN-" + Integer.toString(i) + "_${app.id}"
	    logDebug "initialize(): adding driver = " + contactName
        
        def contactDev = getChildDevice(contactName)
	    if(!contactDev) contactDev = addChildDevice("TRATO", "MolSmart GW3 - IR Universal(Learning)", contactName, null, [name: "MolSmGW3 IR-UN" + Integer.toString(i), inputNumber: thisName])
